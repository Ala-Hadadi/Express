...
app.get("/:category/:subcategory/:id", (req, res) => {
  ...
  const categoryMatch = storeData.find((cat) => cat.category === category);
  if (!categoryMatch) {
    return res.status(404).json({ message: "didn't find the category" });
  }
  const subcategoryMatch = categoryMatch.subcategories.find(
    (subcat) => subcat.subcategory === subcategory
  );
  if (!subcategoryMatch) {
    return res.status(404).json({ message: "didn't find the subcategory" });
  }
  const productMatch = subcategoryMatch.products.find(
    (product) => product.id === proId
  );
  if (!productMatch) {
    return res.status(404).json({ message: "didn't find the product" });
  }
